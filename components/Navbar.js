  ...
  return (
    <nav className='bg-gray-900 shadow-xl shadow-white text-white flex justify-between items-center px-4 md:h-16'>
      ...
      <div className='relative flex justify-center items-center  md:block gap-4'>
        {session && (
          <span className="mr-4 text-green-300 font-semibold">Logged in as {session.user.name}</span>
        )}
        {session && <>
          ...
          <div
            id="dropdown"
            className={`z-10 ${showdropdown ? "" : "hidden"} fixed right-[16px] top-[65px] bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}
            onMouseLeave={() => {
